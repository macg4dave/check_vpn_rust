...
mod tests {
    ...
    #[test]
    fn extract_line_col_returns_some_for_truncated() {
        let doc = "<root><child>oops"; // missing closing >
        let res = extract_line_col("error", doc);
        assert!(res.is_some());
        let (line, col) = res.unwrap();
        assert_eq!(col, 1);
        assert_eq!(line, doc.lines().count());
    }

    #[test]
    fn extract_line_col_none_for_wellformed() {
        let doc = "<root><child>ok</child></root>";
        let res = extract_line_col("error", doc);
        assert!(res.is_none());
    }
    ...
    fn serialize_deserialize_roundtrip() {
        let p = P { x: "hi".into(), n: 42 };
        let xml = serialize_xml(&p).expect("serialize");
        let p2: P = deserialize_xml(&xml).expect("deserialize");
        assert_eq!(p, p2);
    }
}
