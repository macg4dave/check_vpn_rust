...
pub fn run(args: Args, cfg: Config) -> Result<()> {
    ...
    if let Err(e) = crate::config::Config::validate_values(eff.interval, &eff.isp_to_check, &eff.action_type, &eff.action_arg) {
        ...
        match crate::config::Config::load() {
            Ok(new_cfg) => {
                if new_cfg != current_cfg {
                    ...
                    match crate::config::Config::validate_values(new_eff.interval, &new_eff.isp_to_check, &new_eff.action_type, &new_eff.action_arg) {
                        Ok(()) => {
                            current_cfg = new_cfg;
                            eff = new_eff;
                        ...
                        Err(e) => {
                            error!("New configuration is invalid, keeping previous config: {}", e);
                        }
                    }
                }
            }
    ...
    FRun: Fn(&actions::Action, bool),
{
    ...
    match networking::is_online_with_ports(&endpoints_ref, eff.connectivity_timeout_secs, &eff.connectivity_ports) {
        Ok(true) => {
            match get_isp_fn() {
                Ok(isp) => {
