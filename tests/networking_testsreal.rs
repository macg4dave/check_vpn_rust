...
fn real_ip_api_returns_200_and_parses() {
    ...
    let isp = check_vpn::ip_api::get_isp_with_client_and_url(&client, &url, 1).expect("expected real ip-api to succeed");
    assert!(!isp.trim().is_empty(), "expected non-empty ISP from real ip-api");
}
...
fn real_timeout_behaviour() {
    ...
    assert!(res.is_err(), "expected timeout to produce an error against slow endpoint");
}
