...
fn missing_required_field_uses_default_on_merge() {
    ...
    path.push(format!("check_vpn_missing_field_{}.xml", std::time::Instant::now().elapsed().as_nanos()));
...
fn env_var_config_override() {
    ...
    let mut path = std::env::temp_dir();
    path.push(format!("check_vpn_env_override_{}.xml", std::time::Instant::now().elapsed().as_nanos()));
    let path_str = path.to_str().unwrap();
    fs::write(path_str, xml).expect("write temp xml");
