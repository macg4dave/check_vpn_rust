...
fn roundtrip_example_config() {
    ...
    assert!(example_path.exists(), "examples/check_vpn.xml must exist for this test");

    let cfg1: check_vpn::config::Config = check_vpn::xml_io::read_xml(example_path.to_str().unwrap()).expect("failed to read example config");
    ...
    let cfg2: check_vpn::config::Config = check_vpn::xml_io::read_xml(path_str).expect("read_xml failed");
    let _ = std::fs::remove_file(path_str);

    assert_eq!(cfg1, cfg2, "roundtrip of example config should preserve values");
}
...
fn malformed_xml_errors_gracefully() {
    ...
    let err = check_vpn::xml_io::read_xml::<check_vpn::config::Config>(path_str).expect_err("expected parse error");
    let _ = std::fs::remove_file(path_str);

    let msg = format!("{}", err);
    assert!(msg.contains("failed to parse xml"), "error should include context prefix: {}", msg);
...
fn strict_unknown_field_errors() {
    ...
    let path_str = path.to_str().expect("failed to get path string");
    std::fs::write(path_str, bad_xml).expect("failed to write bad xml");
    let err = check_vpn::xml_io::read_xml::<check_vpn::config::Config>(path_str).expect_err("expected unknown field error");
    let _ = std::fs::remove_file(path_str);
    let msg = format!("{}", err);
    assert!(msg.contains("unknown"), "strict mode should error on unknown field: {}", msg);
}
